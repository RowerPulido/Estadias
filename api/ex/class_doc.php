<?php
	class Doc
	{
		...
		public function get_info($grupo)
		{
			require_once('connection.php');
			//create lis of states
			$list = array();
			...
			$connection = get_connection();
			//query
			$query = 'select d.id, tod.name, d.status from documentos d join typesofdocs tod on d.type = tod.id where d.alumno = ?;';
			//command
			$command = $connection->prepare($query);
			if ($command === false)
			{
			...
			while($command->fetch())
			{
				array_push($list, new Doc($id, $nombre,$status));
			}
			//return list of cities
			return $list;
		}
		...
		public function get_info_por_alumno($matricula)
		{
			...
			$connection = get_connection();
			//query
			$query = 'select d.id, tod.name, d.status, u.fecha_actualizacon, d.ubicacion from documentos d join typesofdocs tod on d.type = tod.id where d.alumno = ?;';
			//command
			$command = $connection->prepare($query);
			if ($command === false)
			{
			...
			$command->execute();
			//link columns to variables
			$command->bind_result($id, $nombre, $status, $fecha, $ruta);
			//read data
			while($command->fetch())
			{
				array_push($list, new Doc($id, $nombre, $status, $fecha, $ruta));
			}
			//return list of cities
			return $list;
		}
